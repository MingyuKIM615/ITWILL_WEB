...
public class FirstServlet extends HttpServlet {
    ...
    // 클라이언트의 GET 방식 요청을 처리하는(응답을 만들어 주는) 메서드.
    // 파라미터 request: 클라이언트가 보낸 요청(request)에 대한 정보들을 가지고 있는 객체.
    // 파라미터 response: 클라이언트로 보낼 응답(response)를 작성할 때 필요한 객체.
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
    	...
    	PrintWriter writer = response.getWriter(); // 응답(response)를 write할 수 있는 객체를 찾음.
    	writer.append("<!doctype html>")
    		...
    		.append("</head>")
    		.append("<body>")
    		.append("<h1>내가 만든 첫번째 서블릿(Servlet)</h1>")
    		.append("<h2>")
